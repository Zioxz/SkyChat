...
public class ChatServiceTests
{
    ...
    {
        var time = ChatService.GetMuteTime(new(), DateTime.Now);
    ...
    {
        var time = ChatService.GetMuteTime(new() { rule1, rule1 }, DateTime.Now);
        Assert.That(time, Is.EqualTo(100).Within(0.00001));
    }
    [Test]
    public void MuteTimeNewThreeTimesRule2()
    {
        var time = ChatService.GetMuteTime(new() { rule2,rule2,rule2 }, DateTime.Now);
        Assert.That(time, Is.EqualTo(27).Within(0.00001));
    }
    [Test]
    public void MuteTimeReducesthirdPerMonth()
    {
        var time = ChatService.GetMuteTime(new() { rule1 }, DateTime.Now - TimeSpan.FromDays(30));
        Assert.That(time, Is.EqualTo(7).Within(0.00001));
    }
    [Test]
    public void MuteTimeReducesthirdPerMonthLong()
    {
        var time = ChatService.GetMuteTime(new() { rule1 }, DateTime.Now - TimeSpan.FromDays(60));
